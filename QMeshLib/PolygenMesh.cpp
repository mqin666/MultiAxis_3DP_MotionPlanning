    ...
    if (this->meshType == WAYPOINT) {
        ...
        for (GLKPOSITION Pos = meshList.GetHeadPosition(); Pos != NULL; ) {
            ...
            for (GLKPOSITION PosEdge = (mesh->GetEdgeList()).GetHeadPosition(); PosEdge != NULL;) {
                ...
                else if ((edge->GetStartPoint()->Jump_preSecEnd == true && edge->GetEndPoint()->Jump_nextSecStart == true) || (edge->GetEndPoint()->Jump_preSecEnd == true && edge->GetStartPoint()->Jump_nextSecStart == true)) {
